...
DirectedGraph.layout(graph, {
  ...
  customOrder: (glGraph, jointGraph, defaultOrder) =>
    minimizeCrossings(glGraph, jointGraph, defaultOrder, {
      parentChildLinks,
      layoutId,
      personById,
      identicalGroupOf,
...
const [leftEl, rightEl] = fromParentX <= toParentX ? [fromEl, toEl] : [toEl, fromEl];
...
new IdenticalLinkShape({
  source: {
    id: linkA.id,
    anchor: { name: 'connectionRatio', args: { ratio: ratioA } },
  },
  target: {
    id: linkB.id,
    anchor: { name: 'connectionRatio', args: { ratio: ratioB } },
  },
});
```
