...
When you set out to evaluate or build a [BPMN modeler UI](https://www.jointjs.com/blog/bpmn-modeling-vs-execution), you will quickly notice that nearly all diagramming products claim to support the BPMN 2.0 specification. **However, that on its own tells you almost nothing about how well the tool will actually serve your users in production.**
...
[In a previous article](https://www.jointjs.com/blog/why-building-a-bpmn-modeler-ui-is-harder-than-it-looks), we walked through the three main challenges of building a BPMN modeler from scratch – **rendering, user experience, and engine integration**. Here, we will take a different angle. Rather than asking *"how hard is this to build?"*, we will ask *"what should the finished product actually do?"*
...
Enforcing these rules interactively – preventing invalid connections, auto-enclosing elements when a pool is dropped, and giving users real-time feedback – requires deep integration between the shape model and the interaction layer. It is one of the areas that diagramming libraries like [JointJS](https://www.jointjs.com) can handle for you, and one of the areas that takes the longest to implement correctly from scratch.
...
- **Pinch-to-zoom and touch pan** for tablet and touchscreen environments (a capability that [was explicitly removed by the bpmn-js library](https://bpmn.io/blog/posts/2024-bpmn-js-17-removing-touch-interaction-support.html) in a recent release).
...
[In our experience](https://www.jointjs.com/blog/the-complexity-of-diagramming), the effort to implement link routing from scratch is comparable to the effort to implement all of the core rendering foundation. That is why relying on a battle-tested library foundation here is almost always the right call.
...
- **Custom drag preview** that snaps to the underlying grid, rather than just a fixed cursor offset. The native [HTML Drag and Drop API](https://developer.mozilla.org/en-US/docs/Web/API/HTML_Drag_and_Drop_API) does not offer this level of control, so a custom implementation is required.
...
Getting property panels right is a significant design investment. JointJS addresses this with its [Inspector component](https://docs.jointjs.com/learn/features/property-editor-and-viewer), which is fully integrated with its graph data model – changes in the panel propagate to the diagram and to the exported XML automatically.
...
The core risk is unoptimized SVG rendering. Even though [SVG is the right choice for a diagramming tool](https://www.jointjs.com/blog/svg-versus-canvas) (since it offers per-element event handling, CSS styling, and accessibility advantages that canvas-based approaches do not), naively rendering all elements as DOM nodes regardless of viewport visibility leads to performance degradation that tends to only surface with large real-world diagrams.

The solution is **virtual rendering** – i.e. dynamically removing elements from the DOM when they get scrolled out from the viewport, and re-adding them when they get scrolled back in. [JointJS implements this out-of-the-box](https://docs.jointjs.com/learn/release-notes/4.2.0/#optimized-rendering-and-memory-management), while bpmn-js does not, which is a [documented limitation](https://forum.bpmn.io/t/slow-rendering-for-big-diagrams/897) when working with diagrams of several hundred elements.
...
| --- | --- |
| BPMN 2.0 shapes | - Complete shape library |
| BPMN 2.0 semantic constraints | - Constraints enforced during user interaction - Pool containment hierarchy - Sequence flow restrictions |
| Canvas navigation | - Zoom and scroll - Infinite canvas - Fit to content - Touch support - Configurable keyboard shortcuts |
| Link routing | - Orthogonal and straight routing - Allows manual editing of connection points - Allows manual editing of vertices - BPMN link styling and markers - Link labels |
| Element palette | - Configurable palette curation - Drag-and-drop with grid snapping - Custom drag preview - Snaplines |
| Undo/redo | - Compound operations - Aware of contained and attached elements |
| Property panel | - Contextual display based on current selection - Configurable fields - Input validation - Conditional visibility - Grouping and ordering - Complex input |
| BPMN 2.0 XML import/export | - Import BPMN 2.0 XML - Export BPMN 2.0 XML - BPMN schema compliance - Engine extensions round-trip safety |
| Process validation | - BPMN structural validation - Engine-specific validation - Allows presenting validation errors in UI |
| Rendering performance at scale | - SVG with virtual rendering |

‍

💡 Save this checklist for later — [download the PDF version](https://4243902.fs1.hubspotusercontent-na1.net/hubfs/4243902/JointJS/bpmn-modeler-ui-feature-checklist.pdf) to share with your team, or use it as a reference when evaluating BPMN modeling tools.
...
Is BPMN 2.0 compliance enough to call a tool production-ready?

What features should a BPMN modeler UI actually have?

How can I compare BPMN modeling tools?

What separates a good BPMN modeler from a merely functional one?

Which features of a good modeler UI hide the most implementation complexity?

Why can canvas navigation cause cascading bugs across other features?

Why is link routing so much harder than it looks?

What exactly does "round-trip safety" mean for BPMN XML, and why does it matter?

What are the two distinct levels of process validation, and how do they differ?

What is "virtual rendering", and why is it important?