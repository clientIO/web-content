...
- speed up the preparation of schematics for individual work permits
- connect the visual output directly to the source data describing the network
- scale the workflow to keep pace with operational demand
- embed the diagram generation capability directly inside the internal work permit application that operations teams already use
...
- **A shape API for building custom elements** — the team defined their own electrical primitives (circuit breakers, disconnectors, transformers, busbars, generators, loads) once and reused them across every generated diagram
- **A programmatic model API** — graphs are constructed in code from Elia's network data, so diagrams are produced from the source of truth rather than drawn by hand
- **Built-in orthogonal link routing and automatic layout** — used as the foundation for arranging connections the way single-line schematics require
- **A library, not a framework** — JointJS embeds directly inside Elia's existing internal web application, alongside the permit logic itself
